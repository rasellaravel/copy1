...
var SemiSticky = function (element, options) {
  ...
  this.init = function () {
    ...
    $(window).on('scroll.semisticky', function () {
      ...
      thresholdCounter = Math.min(
        Math.max(thresholdCounter - delta, -options.scrollThreshold),
        options.scrollThreshold
      );
      var newOffset;

      if (
        Math.abs(thresholdCounter) >= options.scrollThreshold ||
        _this.state == 'scrolling'
      ) {
        if (delta > 0 && _this.state !== 'hidden') {
          if (_this.currentOffsetAmount > -options.offsetLimit) {
            _this.currentOffsetAmount = Math.max(
              _this.currentOffsetAmount - delta,
              -options.offsetLimit
            );
          ...
          if (_this.currentOffsetAmount < 0) {
            _this.currentOffsetAmount = Math.min(
              _this.currentOffsetAmount - delta,
              0
            );
            _this.element.css('top', _this.currentOffsetAmount);
